...
function fixture(over: Partial<RoomDescriptor> = {}) {
...
describe('room descriptor events', () => {
  it('is an ephemeral kind, so no relay keeps a record that the room existed', () => {
    expect(KINDS.DESCRIPTOR).toBeGreaterThanOrEqual(20000)
    expect(KINDS.DESCRIPTOR).toBeLessThan(30000)
  })

  it('round-trips a room that names no forwarder', () => {
    const { roomId, roomKey, deviceSk, descriptor } = fixture()
  ...
  it('round-trips a room that names one forwarder', () => {
    const { roomId, roomKey, deviceSk, descriptor } = fixture({
  ...
  it('round-trips a room that names several forwarders, in order', () => {
    const forwarders = [
      { url: 'wss://fwd-a.example/fwd', pubkey: 'aa'.repeat(32) },
      { url: 'wss://fwd-b.example/fwd', label: 'Community box' },
      { url: 'ws://fwd-c.local:7788' },
    ]
    const { roomId, roomKey, deviceSk, descriptor } = fixture({ forwarders })
    const event = encodeDescriptorEvent(descriptor, { roomId, roomKey, deviceSk })
    const decoded = decodeDescriptorEvent(event, { roomId, roomKey, now: NOW })
  ...
  it('never lets a forwarder entry carry the room key', () => {
  ...
  it('strips an unknown field a second implementation added to a forwarder entry', () => {
  ...
  it('projects ice servers onto their known fields', () => {
    const { roomId, roomKey, deviceSk, descriptor } = fixture({
      iceServers: [
  ...
  it('normalises a forwarder pubkey to lower case at the boundary', () => {
    const { roomId, roomKey, deviceSk, descriptor } = fixture({
  ...
  it('leaves the room id the only thing a relay can read', () => {
    const { roomId, roomKey, deviceSk, descriptor } = fixture({
  ...
  it('drops a malformed forwarder entry rather than the whole descriptor', () => {
    const { roomId, roomKey, deviceSk, descriptor } = fixture()
    const mixed = {
      ...descriptor,
  ...
  it('treats a missing forwarder list as an empty one', () => {
    const { roomId, roomKey, deviceSk, descriptor } = fixture()
...
describe('room descriptor rejection', () => {
  const encoded = (over: Partial<RoomDescriptor> = {}) => {
    const f = fixture(over)
    return { ...f, event: encodeDescriptorEvent(f.descriptor, f) }
  }

  it('refuses a descriptor encrypted to another room key', () => {
    const { roomId, event } = encoded()
  ...
  it('refuses a descriptor tagged for another room', () => {
    const { roomKey, event } = encoded()
    const other = deriveRoom(new Uint8Array(32).fill(4))
    expect(decodeDescriptorEvent(event, { roomId: other.roomId, roomKey, now: NOW })).toBeNull()
  })

  it('refuses a tampered signature', () => {
    const { roomId, roomKey, event } = encoded()
    expect(decodeDescriptorEvent({ ...event, sig: '00'.repeat(64) }, { roomId, roomKey, now: NOW })).toBeNull()
  })

  it('refuses a descriptor signed by a device the credential does not name', () => {
    const { roomId, roomKey, descriptor } = fixture({ forwarders: [{ url: 'wss://mine.example/fwd' }] })
  ...
  it('refuses a descriptor whose credential has expired', () => {
    const { roomId, roomKey, event } = encoded()
    expect(decodeDescriptorEvent(event, { roomId, roomKey, now: NOW + 7200 })).toBeNull()
  })

  it('refuses a descriptor stamped beyond clock skew', () => {
    ...
    const { roomId, roomKey, event } = encoded({ updatedAt: NOW + 86_400 })
    expect(decodeDescriptorEvent(event, { roomId, roomKey, now: NOW })).toBeNull()
  })
  ...
  it('never throws on garbage, because it runs inside a subscription handler', () => {
