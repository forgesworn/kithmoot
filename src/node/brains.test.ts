...
it('returns room history as a correlated snapshot without replaying messages as new requests', async () => {
  ...
  const conversation = vi.fn(() => ({ messages: () => [message, { ...message, id: 'reaction', reaction: { emoji: '👍' } }] }))
  const runtime = {
  ...
  try {
    ...
    expect(events).toContainEqual({ type: 'history-snapshot', id: 'request-1', room: 'c'.repeat(64), channel: 'chat', messages: [message] })
