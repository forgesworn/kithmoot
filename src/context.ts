...
import { encryptEnvelope, decryptEnvelope, uploadEnvelope, normaliseBlossomServer } from './attachment.js'

export type ContextScope = 'personal' | 'kin' | 'kith'
export type ContextRole = 'read' | 'write'
export interface ContextIdentity extends ParticipantIdentity {
...
function policy(value: unknown): { event: Event; body: ContextPolicy } {
  ...
  for (const g of p.grants) {
    ...
    seen.add(g.subject)
    if (g.agent) assert(g.agent.agent === g.subject && verifyAgentOwnership(g.agent, { agent: g.subject, now: e.created_at }).ok, 'Invalid agent ownership proof.')
    assert(p.scope !== 'personal' || g.agent?.principal === p.owner, 'Personal context can only grant access to its owner’s agents.')
  }
...
function role(p: ContextPolicy, subject: string, at: number): ContextRole | undefined {
  ...
  if (!g || g.agent && !verifyAgentOwnership(g.agent, { agent: g.subject, now: at }).ok) return undefined
  return g.role
}
...
export class ContextVault {
  ...
  #visible(p: ContextPolicy): boolean {
    return !!role(p, this.#identity.pubkey, this.#now()) &&
  ...
  #get(collection: string): OpenCollection {
    ...
    assert(entry && this.#visible(policy(entry.policy).body), 'Context collection not available.')
    return entry
  }
  ...
  grants(collection: string): ContextGrant[] { return structuredClone(policy(this.#get(collection).policy).body.grants) }

  list(): Omit<ContextView, 'records'>[] {
    return [...this.#collections.values()].filter(e => this.#visible(policy(e.policy).body)).map(e => {
      const { records: _, ...view } = this.read(policy(e.policy).body.collection)
      return view
    })
  }
  read(collection: string, query = ''): ContextView {
    ...
    const p = s.policy
    const rows = s.body.records.map(e => record(e, p.owner, p.collection))
    ...
    return { id: p.collection, owner: p.owner, title: p.title, scope: p.scope, ...(p.room ? { room: p.room } : {}), epoch: p.epoch,
      ...
      role: role(p, this.#identity.pubkey, this.#now())!, uploaded: !!entry.pointer, records }
  }
  ...
  async append(collection: string, expectedHead: string, input: Omit<ContextRecord, 'id'>): Promise<ContextView> {
    ...
    const p = policy(before.policy).body
    assert(role(p, this.#identity.pubkey, this.#now()) === 'write', 'Context write permission required.')
    ...
    record(r, p.owner, collection)
    const next = await this.#build(before.policy, [...snapshot(before.snapshot).body.records, r], before)
  ...
  async setGrants(collection: string, expectedHead: string, grants: ContextGrant[]): Promise<ContextView> {
    ...
    const next = await this.#build(nextPolicy, snapshot(before.snapshot).body.records, before, id())
    assert(this.#get(collection).snapshot.id === expectedHead, 'Context changed while signing; retry from the current head.')
    this.#collections.set(collection, next)
    return this.read(collection)
  }
  ...
  async upload(collection: string, server: string): Promise<ContextPointer> {
    ...
    const entry = this.#get(collection)
    assert(role(policy(entry.policy).body, this.#identity.pubkey, this.#now()) === 'write', 'Context write permission required to upload.')
  ...
  async #access(entry: OpenCollection, recipient: string, location: ContextPointer): Promise<Event> {
    const p = policy(entry.policy).body
    assert(role(p, this.#identity.pubkey, this.#now()) === 'write' && role(p, recipient, this.#now()), 'An active write grant and recipient grant are required to share access.')
  ...
  async #openAccess(input: unknown): Promise<{ body: AccessBody; access: Event }> {
    ...
    const p = policy(body.policy).body
    assert(body.v === 1 && HEX.test(body.key) && HEX.test(body.head), 'Invalid context access.')
    assert(this.#visible(p) && role(p, access.pubkey, access.created_at) === 'write', 'Context collection not available.')
  ...
  #accept(body: AccessBody, envelope: Uint8Array): ContextView {
    ...
    const s = snapshot(parse(dec.decode(opened.source)))
