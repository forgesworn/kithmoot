...
describe('agents in the roster', () => {
  it('shows a device that declares itself an agent as one, on the participant it belongs to', async () => {
    ...
    expect(views.find((v) => v.participant === agent.participant)?.requestReceipts).toBeUndefined()
    await agent.setRequestReceipts(true)
    await settle()
    expect(person.participants().find(v => v.participant === agent.participant)?.requestReceipts).toBe(true)
    await agent.setRequestReceipts(false)
    await settle()
    expect(person.participants().find(v => v.participant === agent.participant)?.requestReceipts).toBeUndefined()
    person.leave()
    agent.leave()
  })
