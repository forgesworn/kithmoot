...
personal = await vault.setGrants(personal.id, personal.head, [{ subject: agent.pubkey, role: 'read', expiresAt: now + 3600, agent: issueAgentOwnership({ principalSk: sk, agent: agent.pubkey, issuedAt: now, expiresAt: now + 86400 }) }])
