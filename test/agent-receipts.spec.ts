...
test('Chip completes by name and @all receives acknowledgements from both agents in the same conversation', async ({ browser, baseURL }) => {
  ...
  try {
    ...
    await input.press('Tab'); await expect(input).toHaveValue('@Chip ')
    await input.pressSequentially('extract these numbers: 3, 7')
    await page.locator('#chatForm button[type=submit]').click()
    await expect.poll(() => addressed[1]).toContain('@Chip extract these numbers: 3, 7')
    ...
    await expect(input).toHaveValue('@all ')
    await input.pressSequentially('please acknowledge the fixture')
    await page.locator('#chatForm button[type=submit]').click()
    const row = page.locator('#chatLog .msg').filter({ hasText: '@all please acknowledge the fixture' })
