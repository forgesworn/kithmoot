...
test('a new room exposes invitations directly, confirms copying honestly and admits a second person', async ({ browser, baseURL }, testInfo) => {
  const { context } = await device(browser, baseURL!)
  const guest = await device(browser, baseURL!)
  await context.addInitScript(() => {
    Object.defineProperty(navigator, 'clipboard', { configurable: true, value: { writeText: async (text: string) => { (window as any).copiedInvite = text } } })
    Object.defineProperty(navigator, 'share', { configurable: true, value: undefined })
  })
  try {
    const page = await context.newPage()
    await page.goto(baseURL!)
    await page.locator('#roomName').fill('The workshop')
    await page.locator('#create').click()
    await expect(page.locator('#arrivalLead')).toContainText('Invite people')
    await page.locator('#displayName').fill('Ada')
    await page.locator('#join').click()
    await expect(page.locator('#invitePeople')).toBeVisible()
    await expect(page.locator('#chatLog')).toContainText('Use Invite people')
    await page.locator('#chatInput').fill('Keep this unfinished welcome')
    await page.locator('#invitePeople').focus()
    await page.locator('#invitePeople').press('Enter')
    const dialog = page.getByRole('dialog', { name: 'Invite people', exact: true })
    ...
    await expect(page.locator('#invitationAvailability')).toContainText('everyone closes the app')
    await dialog.getByRole('button', { name: 'Copy link', exact: true }).click()
    await expect(page.locator('#inviteStatus')).toHaveText('Link copied. Share it with the people you want to invite.')
    const link = await page.locator('#shareUrl').inputValue()
    expect(await page.evaluate(() => (window as any).copiedInvite)).toBe(link)
    ...
    await expect(dialog).toBeVisible()
    // A rejected browser clipboard and an unavailable fallback must not say copied.
    await page.evaluate(() => {
      Object.defineProperty(navigator, 'clipboard', { configurable: true, value: { writeText: async () => { throw new Error('Unavailable') } } })
      document.execCommand = () => false
    })
    await dialog.getByRole('button', { name: 'Copy link', exact: true }).click()
    await expect(page.locator('#inviteStatus')).toContainText('Automatic copy was unavailable')
    await page.screenshot({ path: testInfo.outputPath('invite-copy-failed.png') })
    await page.evaluate(() => {
      Object.defineProperty(navigator, 'clipboard', { configurable: true, value: { writeText: async (text: string) => { (window as any).copiedInvite = text } } })
    })
    await dialog.getByRole('button', { name: 'Copy link', exact: true }).click()
    await expect(page.locator('#inviteStatus')).toContainText('Link copied.')
    for (const colorScheme of ['light', 'dark'] as const) {
      await page.emulateMedia({ colorScheme })
      for (const width of [320, 390, 1440]) {
        await page.setViewportSize({ width, height: 740 })
        expect(await dialog.evaluate(el => el.scrollWidth <= el.clientWidth)).toBe(true)
        await page.screenshot({ path: testInfo.outputPath(`invite-${colorScheme}-${width}.png`) })
      }
    }
    await page.locator('#shareQrDetails summary').click()
    await expect.poll(() => page.locator('#shareQr').evaluate((canvas: HTMLCanvasElement) => canvas.getContext('2d')!.getImageData(0, 0, canvas.width, canvas.height).data.some(value => value !== 0))).toBe(true)
    ...
    await expect(page.locator('#invitePeople')).toBeFocused()
    await expect(page.locator('#chatInput')).toHaveValue('Keep this unfinished welcome')
    await page.evaluate(() => { document.documentElement.style.fontSize = '' })
    await page.locator('#invitePeople').click()
    await expect(dialog).toBeVisible()
    await expect(page.locator('#copyShare')).toBeFocused()
    await page.mouse.click(1, 1)
    await expect(dialog).not.toBeVisible()
    await page.locator('#roomMenu').click()
    await expect(page.locator('#roomSheet #shareUrl')).toHaveValue(link)
    expect(await page.locator('#shareUrl').count()).toBe(1)
